...
export default function Listing() {
  ...
  useEffect(()=>{
    const getList = async()=>{
        const res = await fetch(`/api/listing/getUserListing/${params.listId}`);
  ...
  const handleSubmitList = async (e) => {
    e.preventDefault();
    const res = await fetch(`/api/listing/updateUserList/${params.listId}`, {
      method: "POST",
      headers: {
    ...
    if (data.success === false) {
      return setImageError("Unable to update list");
    }
    navigate('/profile');
  };
  return (
